...
module.exports = {
  // Ad-hoc runs of specific features: `npx cucumber-js -p single <feature>`.
  // (The default profile's `paths` MERGE with CLI arguments, so without this
  // profile a single-feature invocation silently runs the whole suite.)
  // Honours @skip so isolated runs match what the suite actually executes.
  single: { ...base, tags: 'not @skip' },
  // CI PR gate: only the journeys tagged @critical, in suite order.
  critical: {
    ...base,
    tags: '@critical and not @skip',
    paths: orderedPaths,
    retry: 1,
    format: [...base.format, 'json:reports/cucumber-report.json'],
  },
  default: {
    ...base,
    tags: 'not @skip',
