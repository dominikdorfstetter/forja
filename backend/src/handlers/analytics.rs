...
pub async fn get_analytics_report(
    ...
    let parsed_start = start_date
        .map(|s| {
            NaiveDate::parse_from_str(&s, "%Y-%m-%d").map_err(|_| {
                ApiError::bad_request("Invalid start_date format (expected YYYY-MM-DD)".to_string())
            })
        })
        .transpose()?;
    let parsed_end = end_date
        .map(|s| {
            NaiveDate::parse_from_str(&s, "%Y-%m-%d").map_err(|_| {
                ApiError::bad_request("Invalid end_date format (expected YYYY-MM-DD)".to_string())
            })
        })
        .transpose()?;
