...
lazy_static::lazy_static! {
    ...
    /// Valid origin pattern: scheme://host[:port], no path/query/fragment
    static ref ORIGIN_REGEX: Regex = Regex::new(r"^https?://[a-zA-Z0-9]([a-zA-Z0-9\-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9\-]*[a-zA-Z0-9])?)*(\:[0-9]{1,5})?$")
        .expect("ORIGIN_REGEX is a valid regex literal");
}
...
/// Validate a URL format (absolute URLs only: http:// or https://)
...
/// Validate a navigation URL — allows both absolute URLs and root-relative paths.
/// Navigation items legitimately link to internal paths like `/blog/my-post`.
pub fn validate_nav_url(url: &str) -> Result<(), ValidationError> {
    if url.is_empty() {
        return Ok(());
    }
    ...
    let is_relative = url.starts_with('/');

    if !is_absolute && !is_relative {
        let mut err = ValidationError::new("invalid_url");
        err.message = Some("URL must start with http://, https://, or /".into());
        return Err(err);
    }
...
/// Validate a single origin (scheme://host[:port], no path/query/fragment).
pub fn validate_origin(origin: &str) -> Result<(), ValidationError> {
    if origin.is_empty() {
        let mut err = ValidationError::new("invalid_origin");
        err.message = Some("Origin cannot be empty".into());
        return Err(err);
    }

    if origin.len() > 2000 {
        let mut err = ValidationError::new("invalid_origin");
        err.message = Some("Origin cannot exceed 2000 characters".into());
        return Err(err);
    }
    ...
    if origin.contains('*') {
        let mut err = ValidationError::new("invalid_origin");
        err.message = Some("Wildcards are not allowed in origins".into());
        return Err(err);
    }
    ...
    if origin.ends_with('/') {
        let mut err = ValidationError::new("invalid_origin");
        err.message = Some("Origin must not end with a trailing slash".into());
        return Err(err);
    }

    // Reject paths (anything after host:port)
    ...
    if !ORIGIN_REGEX.is_match(origin) {
        let mut err = ValidationError::new("invalid_origin");
        err.message = Some("Origin must be scheme://host[:port] (e.g. https://example.com)".into());
        return Err(err);
    }
...
/// Validate a list of allowed origins for site settings.
pub fn validate_allowed_origins(origins: &[String]) -> Result<(), ValidationError> {
...
pub fn validate_storage_quota_bytes(value: i64) -> Result<(), ValidationError> {
    ...
    const MAX: i64 = 1_099_511_627_776; // 1 TB

    if !(MIN..=MAX).contains(&value) {
        let mut err = ValidationError::new("invalid_storage_quota");
        err.message = Some(format!("Storage quota must be between {MIN} and {MAX} bytes").into());
        return Err(err);
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_validate_storage_quota_bytes() {
        assert!(super::validate_storage_quota_bytes(99_999_999).is_err()); // below 100 MB
        assert!(super::validate_storage_quota_bytes(104_857_600).is_ok()); // exactly 100 MB
        assert!(super::validate_storage_quota_bytes(1_073_741_824).is_ok()); // 1 GB
    ...
    fn test_validate_origin_valid() {
        ...
        assert!(validate_origin("https://staging.myblog.com").is_ok());
        assert!(validate_origin("https://my-site.example.co.uk").is_ok());
        assert!(validate_origin("http://192.168.1.1:8080").is_ok());
        assert!(validate_origin("https://sub.domain.example.com:443").is_ok());
    }
    ...
    fn test_validate_origin_invalid() {
        ...
        assert!(validate_origin("ws://example.com").is_err());
    }
