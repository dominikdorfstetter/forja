...
impl SortParams {
    ...
    /// Build a safe `column DIRECTION` fragment for use in ORDER BY.
    ///
    /// `column` must be `&'static str` (e.g. from a match on whitelisted values)
    /// to guarantee at compile time that no user input flows into the SQL column position.
    pub fn order_clause(&self, column: &'static str) -> String {
        format!("{} {}", column, self.direction())
    }
}

/// Build a safe `column DIRECTION` fragment from a raw sort direction input.
///
/// Use this in model functions that receive raw `sort_dir: Option<&str>` instead
/// of a `SortParams`. `column` must be `&'static str` to prevent SQL injection.
pub fn order_clause(column: &'static str, sort_dir: Option<&str>) -> String {
...
mod tests {
    ...
    #[test]
    fn order_clause_combines_column_and_direction() {
        let sort = SortParams {
            sort_by: Some("slug".into()),
            sort_dir: Some("asc".into()),
        };
        assert_eq!(sort.order_clause("t.slug"), "t.slug ASC");
    }

    #[test]
    fn order_clause_defaults_direction_to_desc() {
        let sort = SortParams {
            sort_by: None,
            sort_dir: None,
        };
        assert_eq!(sort.order_clause("t.created_at"), "t.created_at DESC");
    }

    #[test]
    fn free_order_clause_asc() {
        assert_eq!(order_clause("m.file_name", Some("asc")), "m.file_name ASC");
    }

    #[test]
    fn free_order_clause_default_desc() {
        assert_eq!(order_clause("m.created_at", None), "m.created_at DESC");
    }

    #[test]
    fn free_order_clause_invalid_defaults_desc() {
        assert_eq!(
            order_clause("m.created_at", Some("bogus")),
            "m.created_at DESC"
        );
    }
