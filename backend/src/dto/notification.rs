...
/// Read/unread split used by the notifications filter pills.
#[derive(Debug, Serialize, ToSchema)]
...
/// Request body for bulk-deleting a list of notifications. The handler
/// scopes the DELETE by the caller's Clerk ID, so a user can't drop
/// someone else's rows even with valid IDs.
#[derive(Debug, Deserialize, ToSchema)]
pub struct BulkDeleteNotificationsRequest {
    pub ids: Vec<Uuid>,
}

/// Response after a destructive notification operation (single or bulk).
#[derive(Debug, Serialize, ToSchema)]
pub struct NotificationDeleteResponse {
    #[schema(example = 7)]
    pub deleted: i64,
}
