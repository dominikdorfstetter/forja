...
pub struct MembershipSummary {
    ...
    pub role: SiteRole,
    /// Resolved permission strings for this role (e.g. "blog:create", "blog:update:own")
...
impl From<MembershipWithSite> for MembershipSummary {
    fn from(row: MembershipWithSite) -> Self {
        use crate::services::permission_service::role_permissions;
        let permissions = role_permissions(&row.role)
            .into_iter()
            .map(|p| p.as_str())
            .collect();
        Self {
