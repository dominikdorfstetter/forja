...
pub struct CreateBlogRequest {
    /// URL-friendly slug. If omitted, auto-generated from `title` or a timestamp fallback.
    #[schema(example = "my-awesome-post")]
    #[validate(length(max = 100))]
    #[validate(custom(function = "validate_slug"))]
    pub slug: Option<String>,

    /// Title used for slug auto-generation when `slug` is not provided.
    #[schema(example = "My Awesome Post")]
...
/// Blog detail response with localizations, taxonomy, and documents
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Blog post with localizations, categories, tags, and documents")]
pub struct BlogDetailResponse {
    ...
    /// Computed OG image URL (cascade: cover image → site default → logo)
    #[schema(example = "https://cdn.example.com/og-image.jpg")]
...
/// Blog counts grouped by workflow status. All five buckets are always
/// returned so the admin filter pills can render zero-counts without a
/// second request.
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Count of blogs per workflow status")]
pub struct BlogStatusCounts {
    pub draft: i64,
    pub in_review: i64,
    pub scheduled: i64,
    pub published: i64,
...
mod tests {
    ...
    fn test_create_blog_request_valid() {
        let request = CreateBlogRequest {
            slug: Some("my-awesome-post".to_string()),
            title: None,
    ...
    fn test_create_blog_request_invalid_slug() {
        let request = CreateBlogRequest {
            slug: Some("My Invalid Slug!".to_string()),
            title: None,
            author: "Jane Doe".to_string(),
            published_date: NaiveDate::from_ymd_opt(2024, 6, 15).unwrap(),
