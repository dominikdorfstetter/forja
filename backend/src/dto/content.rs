...
impl From<ContentLocalization> for LocalizationResponse {
    fn from(loc: ContentLocalization) -> Self {
        // Compute excerpt fallback from body when excerpt is empty
        let excerpt = match &loc.excerpt {
            Some(e) if !e.trim().is_empty() => Some(e.clone()),
            _ => loc.body.as_ref().map(|b| {
                crate::utils::excerpt::compute_excerpt(
                    b,
                    crate::utils::excerpt::DEFAULT_EXCERPT_LEN,
                )
            }),
        };
