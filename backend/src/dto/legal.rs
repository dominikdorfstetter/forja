...
impl From<LegalDocumentWithContent> for LegalDocumentResponse {
    fn from(doc: LegalDocumentWithContent) -> Self {
        Self {
            id: doc.id,
            cookie_name: doc.cookie_name,
            document_type: doc.document_type,
            status: doc.status.unwrap_or_default(),
            slug: doc.slug,
            version: doc.version,
            publish_start: doc.publish_start,
            publish_end: doc.publish_end,
            created_at: doc.created_at,
            updated_at: doc.updated_at,
        }
    }
...
/// Full legal document detail with content localizations (for editor page)
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Legal document with all localizations for editing")]
pub struct LegalDocumentFullDetailResponse {
    pub id: Uuid,
    pub content_id: Uuid,
    pub cookie_name: String,
    pub document_type: LegalDocType,
    ...
    pub doc_localizations: Vec<LegalDocLocalizationResponse>,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

/// Legal document version summary
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Legal document version summary")]
pub struct LegalVersionResponse {
    pub id: Uuid,
    pub version: i32,
    pub status: ContentStatus,
    pub created_at: DateTime<Utc>,
