...
pub struct CreatePageRequest {
    ...
    /// URL-friendly slug. If omitted, auto-generated from `route`.
    #[schema(example = "about")]
...
pub struct PageDetailResponse {
    ...
    /// Computed OG image URL (cascade: site default → logo)
...
/// Page counts grouped by workflow status. All five buckets are always
/// returned so the admin filter pills can render zero-counts without a
/// second request.
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Count of pages per workflow status")]
pub struct PageStatusCounts {
    pub draft: i64,
    pub in_review: i64,
    pub scheduled: i64,
    pub published: i64,
