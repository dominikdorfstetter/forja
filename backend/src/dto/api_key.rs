...
pub struct CreateApiKeyRequest {
    ...
    /// Quota: max requests per hour (default 1000)
    #[validate(range(
        min = 1,
        max = 1000000,
        message = "Hourly quota must be between 1 and 1000000"
    ))]
    pub quota_hourly: Option<i32>,

    /// Quota: max requests per day (default 10000)
    #[validate(range(
        min = 1,
        max = 10000000,
        message = "Daily quota must be between 1 and 10000000"
    ))]
    pub quota_daily: Option<i32>,

    /// Quota: max requests per month (default 100000)
    #[validate(range(
        min = 1,
        max = 100000000,
        message = "Monthly quota must be between 1 and 100000000"
    ))]
...
/// Quota window status in the usage summary.
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Quota window status")]
pub struct QuotaWindowResponse {
    pub limit: u32,
    pub used: u32,
    pub remaining: u32,
...
/// One day of aggregated usage in the history.
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "Daily usage summary")]
...
/// Usage summary combining real-time quota info, historical daily data,
/// and all-time totals.
#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
#[schema(description = "API key usage summary with quota tracking")]
pub struct UsageSummaryResponse {
    pub quota: UsageSummaryQuota,
    pub history: UsageSummaryHistory,
    pub totals: UsageSummaryTotals,
}

#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
pub struct UsageSummaryQuota {
    pub hourly: Option<QuotaWindowResponse>,
    pub daily: Option<QuotaWindowResponse>,
    pub monthly: Option<QuotaWindowResponse>,
}

#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
pub struct UsageSummaryHistory {
