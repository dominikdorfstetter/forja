...
///
/// **Contract** — this endpoint is single-path:
///
/// - `GET /sites/{site_id}/redirects/lookup?path=…`
/// - **200** when an active redirect exists for `path`, with body
///   `{ destination_path, status_code }`.
/// - **404** when no active redirect exists (a plain
///   [`ProblemDetails`](crate::errors::ProblemDetails) body — **never**
///   `200` with a `null` payload, never a `{ redirects: [] }` list).
/// - `status_code` is always one of `301 | 302 | 307 | 308`. This is
///   enforced at three layers: the create/update DTO validator
///   ([`validate_redirect_status_code`]), the `chk_redirect_status_code`
///   DB CHECK constraint, and the SDK literal-union type. Consumers may
///   rely on the value without runtime coercion.
///
/// The admin list endpoint (`GET /sites/{site_id}/redirects`) is the
/// only redirect endpoint that returns a collection — there is no bulk
/// lookup. See issue #743.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
#[schema(
...
pub struct RedirectLookupResponse {
    #[schema(example = "/new-blog-post")]
    pub destination_path: String,
    /// Redirect HTTP status code. Always one of `301 | 302 | 307 | 308`.
...
/// Validate redirect status code. Must be one of `301 | 302 | 307 | 308`.
///
/// Mirrors the `chk_redirect_status_code` DB CHECK constraint so the
/// validator and persistence layer reject the same domain. See
/// [`RedirectLookupResponse`] for the consumer-facing contract.
fn validate_redirect_status_code(code: i16) -> Result<(), validator::ValidationError> {
    match code {
        ...
        _ => {
            ...
            err.message = Some("Status code must be one of 301, 302, 307, or 308".into());
            Err(err)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_create_redirect_accepts_307_and_308() {
        for code in [307_i16, 308] {
            let req = CreateRedirectRequest {
                source_path: "/old-page".to_string(),
                destination_path: "/new-page".to_string(),
                status_code: code,
            ...
            assert!(
                req.validate().is_ok(),
                "status_code {code} should pass validation"
            );
        }
    }

    #[test]
    fn test_create_redirect_rejects_200_and_other_3xx() {
        for code in [200_i16, 303, 305, 306, 309, 400, 500] {
            let req = CreateRedirectRequest {
                source_path: "/old-page".to_string(),
                destination_path: "/new-page".to_string(),
                status_code: code,
            ...
            assert!(
                req.validate().is_err(),
                "status_code {code} should fail validation"
            );
        }
    }
