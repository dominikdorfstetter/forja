...
impl Tag {
    ...
    /// Assign a tag to content. Idempotent — re-assigning is a no-op.
    ...
    pub async fn remove_from_content(
        ...
        let result = sqlx::query("DELETE FROM content_tags WHERE content_id = $1 AND tag_id = $2")
            .bind(content_id)
            .bind(tag_id)
            .execute(pool)
            .await?;
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::not_found("Tag assignment not found")
                .with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("tag_assignment"));
        }
    ...
    pub async fn remove_from_content(
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::not_found("Category assignment not found")
                .with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("category_assignment"));
        }
