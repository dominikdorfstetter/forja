...
impl User {
    ...
    pub async fn find_by_id(pool: &PgPool, id: Uuid) -> Result<Self, ApiError> {
        ...
        .fetch_optional(pool)
        .await?
        .ok_or_else(|| ApiError::not_found(format!("User with ID {} not found", id)).with_code(codes::ENTITY_NOT_FOUND)
    ...
    pub async fn find_by_username(pool: &PgPool, username: &str) -> Result<Self, ApiError> {
        ...
        .fetch_optional(pool)
        .await?
        .ok_or_else(|| ApiError::not_found(format!("User '{}' not found", username)).with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("user"))?;
    ...
    pub async fn find_by_email(pool: &PgPool, email: &str) -> Result<Self, ApiError> {
        ...
        .fetch_optional(pool)
        .await?
        .ok_or_else(|| ApiError::not_found(format!("User with email '{}' not found", email)).with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("user"))?;
    ...
    pub async fn delete(pool: &PgPool, id: Uuid) -> Result<(), ApiError> {
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::not_found(format!("User with ID {} not found", id)).with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("user"));
        }
