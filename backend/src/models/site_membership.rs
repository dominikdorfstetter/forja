...
impl SiteRole {
    ...
    /// The API-key permission this role effectively grants its holder.
    ///
    /// Thin alias over [`role_permission::effective`] — the single home for the
    /// role → permission vocabulary. See that module for why the *effective*
    /// permission differs from the key-minting [`role_permission::creation_cap`].
    ///
    /// [`role_permission::effective`]: crate::models::role_permission::effective
    /// [`role_permission::creation_cap`]: crate::models::role_permission::creation_cap
    pub fn to_api_key_permission(&self) -> ApiKeyPermission {
        crate::models::role_permission::effective(self)
    }
}
...
impl SiteMembership {
    ...
    /// Update the role of an existing membership, scoped to its site.
    ///
    /// `site_id` is part of the WHERE clause so a `member_id` that belongs to
    /// a different site matches zero rows and returns 404 *before any write* —
    /// callers route `member_id` through a path-supplied `site_id` they only
    /// need permission on, so an unscoped update is a cross-tenant write.
