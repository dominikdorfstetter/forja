...
/// Map API/serde page type name (PascalCase) to PostgreSQL enum text value.
...
impl Page {
    ...
    pub async fn find_all_for_site_filtered(
        ...
        if search.is_some() {
            where_clauses.push(format!(
                "(p.id::text ILIKE '%' || ${bind_idx} || '%' OR p.route ILIKE '%' || ${bind_idx} || '%' OR c.slug ILIKE '%' || ${bind_idx} || '%')"
            ));
            bind_idx += 1;
        }
        ...
        let order_dir = match sort_dir.unwrap_or("asc") {
            "desc" | "DESC" => "DESC",
            _ => "ASC",
        };
        ...
        let mut query = sqlx::query_as::<_, PageWithContent>(&sql)
            .bind(site_id)
            .bind(limit)
            .bind(offset);
    ...
    /// Count pages for a site with optional search and filter
...
impl PageSection {
    ...
    /// Batch-reorder page sections for a page within a single transaction
