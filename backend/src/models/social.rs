...
impl SocialLink {
    ...
    pub async fn find_all_for_site(pool: &PgPool, site_id: Uuid) -> Result<Vec<Self>, ApiError> {
        let links = sqlx::query_as::<_, Self>(
            ...
            FROM social_links
            WHERE site_id = $1 AND is_active = TRUE AND is_deleted = FALSE
    ...
    pub async fn reorder_for_site(
        ...
        for (id, display_order) in &items {
            let result = sqlx::query(
                "UPDATE social_links SET display_order = $1, updated_at = NOW() WHERE id = $2 AND site_id = $3 AND is_deleted = FALSE",
            )
            .bind(display_order)
            .bind(id)
            .bind(site_id)
    ...
    /// Soft delete a social link (recoverable via trash)
    ...
    pub async fn restore(pool: &PgPool, id: Uuid) -> Result<(), ApiError> {
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::not_found(format!(
                "Social link with ID {} not found or not deleted",
                id
            ))
            .with_code(codes::ENTITY_NOT_FOUND)
            .with_entity_type("social_link"));
    ...
    pub async fn permanent_delete(pool: &PgPool, id: Uuid) -> Result<(), ApiError> {
        let result = sqlx::query("DELETE FROM social_links WHERE id = $1 AND is_deleted = TRUE")
            .bind(id)
            .execute(pool)
            .await?;
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::not_found(format!(
                "Social link with ID {} not found in trash",
                id
            ))
            .with_code(codes::ENTITY_NOT_FOUND)
            .with_entity_type("social_link"));
    ...
    /// Find a deleted social link by ID (for trash operations)
    pub async fn find_deleted_by_id(pool: &PgPool, id: Uuid) -> Result<Self, ApiError> {
        ...
        .await?
        .ok_or_else(|| {
            ApiError::not_found(format!("Social link with ID {} not found in trash", id))
                .with_code(codes::ENTITY_NOT_FOUND)
                .with_entity_type("social_link")
        })
    }
