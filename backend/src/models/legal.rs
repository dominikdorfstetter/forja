...
impl LegalDocument {
    ...
    /// Count legal documents for a site with optional search filter
    pub async fn count_for_site_filtered(
        ...
        if search.is_some() {
            where_clauses.push(format!("ld.cookie_name ILIKE '%' || ${bind_idx} || '%'"));
            bind_idx += 1;
        }
        let _ = bind_idx;
    ...
    /// Find all legal documents for a site with search, sort, and pagination
