...
impl BulkContentService {
    ...
    pub async fn process_bulk_operation(
        ...
        for result in &resp.results {
            if result.success {
                ...
                webhook_service::dispatch(pool, site_id, &webhook_event, result.id, &payload).await;
            }
        }
    ...
    pub async fn bulk_update_status(
        ...
        for &(entity_id, content_id) in content_ids {
            // Acquire a pooled connection per item now that the spine helper
            // takes an executor (#863); each item stays independent.
            let update = async {
