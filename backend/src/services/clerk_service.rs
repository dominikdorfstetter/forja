...
impl ClerkService {
    /// Construct the Clerk client. Returns `Err` rather than panicking if the
    /// HTTPS-only HTTP client can't be built, so the caller (the binary) decides
    /// how to fail — the service layer never panics on a recoverable error.
    pub fn new(secret_key: String) -> Result<Self, ApiError> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                ApiError::internal(format!("Failed to build HTTPS-only Clerk HTTP client: {e}"))
            })?;
        Ok(Self {
            secret_key,
            client,
