...
interface CreateTemplateWizardProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: Omit<CreateContentTemplateRequest, 'site_id'>) => void;
  loading?: boolean;
}
...
export default function CreateTemplateWizard({ open, onClose, onSubmit, loading }: CreateTemplateWizardProps) {
  ...
  return (
    <FormDialog
      ...
      onClose={onClose}
      icon="dynamic_form"
      title={t('contentTemplates.wizard.title')}
      maxWidth={state.method === 'template' ? 'md' : 'sm'}
      ...
      actions={
        <>
          ...
          {state.activeStep === 1 && (
            <M3Button variant="outlined" size="sm" onClick={() => dispatch({ type: 'GO_BACK' })} disabled={loading}>
              {t('common.actions.back')}
            </M3Button>
          )}
          {state.activeStep === 1 && state.method === 'scratch' && (
            <M3Button variant="filled" size="sm" onClick={handleScratchSubmit} disabled={!state.name.trim() || loading}>
              {loading ? t('common.actions.saving') : t('common.actions.create')}
            </M3Button>
          )}
          {state.activeStep === 1 && state.method === 'template' && (
            <M3Button variant="filled" size="sm" onClick={handleTemplateConfirm} disabled={!state.selectedTemplate || loading}>
              {loading ? t('common.actions.saving') : t('common.actions.create')}
            </M3Button>
          )}
          {state.activeStep === 1 && state.method === 'import' && state.parsed && (
            <M3Button variant="filled" size="sm" onClick={handleImportConfirm} disabled={!state.editName.trim() || loading}>
              {loading ? t('common.actions.saving') : t('common.actions.create')}
            </M3Button>
          )}
        </>
