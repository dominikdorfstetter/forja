    ...
    <>
      {!parsed ? (
        <>
          ...
          <input ref={fileInputRef} type="file" accept=".md,.markdown" hidden aria-label={t('common.actions.selectFile')} onChange={(e) => { const file = e.target.files?.[0]; if (file) processFile(file); }} />
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <TextField label={t('forms.contentTemplate.fields.slugPrefix')} value={editSlugPrefix} onChange={(e) => onEditSlugPrefixChange(e.target.value)} fullWidth slotProps={{
          ...
          <TextField
            ...
            multiline
            slotProps={{
              input: { readOnly: true, sx: { fontFamily: 'monospace', maxHeight: 200, overflow: 'auto' } }
            }}
          />
        </Box>
      )}
