...
describe('WebhookFormDialog', () => {
  ...
  it('shows template picker in create mode', () => {
    renderWithProviders(
      <WebhookFormDialog open onClose={vi.fn()} onSubmitCreate={vi.fn()} />,
    );
    expect(screen.getByRole('radiogroup')).toBeInTheDocument();
    expect(screen.getByTestId('template-card-vercel')).toBeInTheDocument();
    expect(screen.getByTestId('template-card-custom')).toBeInTheDocument();
  });

  it('does not show template picker in edit mode', () => {
    renderWithProviders(
      <WebhookFormDialog
        open
        webhook={existingWebhook}
  ...
  it('shows debounce field', () => {
    renderWithProviders(
      <WebhookFormDialog open onClose={vi.fn()} onSubmitCreate={vi.fn()} />,
    );
    expect(screen.getByLabelText(/debounce/i)).toBeInTheDocument();
  });
});
