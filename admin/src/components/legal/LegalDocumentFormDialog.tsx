...
import { requiredString, siteIdsField, formResolver} from '@/utils/validation';
...
export default function LegalDocumentFormDialog({ open, siteId: _siteId, document, onSubmit, onClose, loading }: LegalDocumentFormDialogProps) {
  ...
  const { register, handleSubmit, reset, control, formState: { errors, isValid } } = useForm<LegalDocFormData>({
    resolver: formResolver(legalDocSchema),
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Controller name="document_type" control={control} render={({ field }) => (
              <TextField select label={t('forms.legal.fields.documentType')} fullWidth {...field}>
                {(['CookieConsent', 'PrivacyPolicy', 'TermsOfService', 'Imprint', 'Disclaimer'] as const).map((type) => (
                  <MenuItem key={type} value={type}>{t(`legal.documentTypes.${type}`)}</MenuItem>
                ))}
              </TextField>
            )} />
            {!document && (
              <Controller name="status" control={control} render={({ field }) => (
                <TextField select label={t('forms.blog.fields.status')} fullWidth {...field}>
                  {(['Draft', 'InReview', 'Scheduled', 'Published', 'Archived'] as const).map((s) => (
                    <MenuItem key={s} value={s}>{t(`common.status.${s === 'InReview' ? 'inReview' : s.toLowerCase()}`)}</MenuItem>
                  ))}
                </TextField>
              )} />
            )}
            {!document && (
              <Controller name="site_ids" control={control} render={({ field }) => (
                <TextField
                  select
                  label={t('forms.blog.fields.siteId')}
                  fullWidth
                  required
                  {...field}
