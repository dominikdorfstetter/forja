...
export default function SetupChecklist({
  ...
  const steps: ChecklistStep[] = useMemo(() => {
    ...
    const base: ChecklistStep[] = [
      ...
      { key: 'preview_site', icon: <PreviewIcon fontSize="small" />, done: done('preview_site'), route: '/site-settings/content', estimateKey: '30sec' },
      ...
      { key: 'customize_settings', icon: <SettingsIcon fontSize="small" />, done: done('customize_settings'), route: '/site-settings', estimateKey: '2min' },
    ];

    if (isTeam) {
      base.push(
        { key: 'invite_team', icon: <GroupAddIcon fontSize="small" />, done: done('invite_team'), route: '/members', estimateKey: '1min' },
        { key: 'setup_workflow', icon: <WorkflowIcon fontSize="small" />, done: done('setup_workflow'), route: '/site-settings/content', estimateKey: '1min' },
      );
    }
