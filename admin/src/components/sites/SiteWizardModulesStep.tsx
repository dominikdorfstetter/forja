...
// Keys must match the `modules` shape in `SiteCreationWizard.tsx`'s Zod
// schema. `portfolio` is the schema key for the CV / portfolio module —
// the older `cv` rendering key didn't match the form state.
const MODULE_DEFS = [
  { key: 'blog' as const, icon: <ArticleIcon />, defaultOn: true },
  { key: 'pages' as const, icon: <WebIcon />, defaultOn: true },
  { key: 'portfolio' as const, icon: <WorkIcon />, defaultOn: false },
  { key: 'legal' as const, icon: <GavelIcon />, defaultOn: false },
  { key: 'documents' as const, icon: <DescriptionIcon />, defaultOn: false },
  { key: 'forms' as const, icon: <DynamicFormIcon />, defaultOn: false },
