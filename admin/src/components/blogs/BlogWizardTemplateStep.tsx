...
interface BlogWizardTemplateStepProps {
...
export function buildMergedTemplates(t: (key: string) => string, siteTemplates: ContentTemplate[]): MergedTemplate[] {
...
export default function BlogWizardTemplateStep({
  selectedTemplate,
  onSelectTemplate,
  templateSearch,
  onSearchChange,
...
}: BlogWizardTemplateStepProps) {
  const { t } = useTranslation();
  const theme = useTheme();

  const mergedTemplates = buildMergedTemplates(t, siteTemplates);
  ...
  return (
    <>
      <TextField
        ...
        onChange={(e) => { onSearchChange(e.target.value); onPageChange(0); onSelectTemplate(null); }}
        sx={{ mb: 2 }}
        slotProps={{
          input: {
            startAdornment: (
              ...
              return (
                ...
                >
                  <Box sx={{ p: 2.5, display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <Typography variant="subtitle1" sx={{ mb: 0.5, fontWeight: 600 }}>{template.name}</Typography>
