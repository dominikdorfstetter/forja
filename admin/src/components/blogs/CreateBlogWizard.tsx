...
import { slugField, requiredString, formResolver} from '@/utils/validation';
...
export default function CreateBlogWizard({
  ...
  const defaultLocaleName = defaultLocale?.name ?? 'English';

  const slugManuallyEdited = useRef(false);
  const { register, handleSubmit, reset: resetForm, formState: { errors }, watch, setValue } = useForm<ScratchFormData>({
    resolver: formResolver(scratchSchema),
    defaultValues: { title: '', slug: '', author: userFullName || '' },
    mode: 'onChange',
  });
  ...
  const handleAiGenerateOutline = async () => {
    ...
    try {
      const result = await aiGenerate('draft_outline', state.aiIdea, defaultLocaleCode);
      dispatch({
  ...
  const handleAiGeneratePost = async () => {
    ...
    try {
      const result = await aiGenerate('draft_post', content, defaultLocaleCode);
  ...
  const handleRegenerate = useCallback(async (field: RegenerateField) => {
    ...
    try {
      if (field === 'all') {
        ...
        const result = await aiGenerate('excerpt', s.aiBody, defaultLocaleCode);
        if (result.excerpt) dispatch({ type: 'SET_AI_EXCERPT', value: result.excerpt });
      } else if (field === 'seo') {
        const result = await aiGenerate('seo', s.aiBody, defaultLocaleCode);
        if (result.meta_title) dispatch({ type: 'SET_AI_META_TITLE', value: result.meta_title });
        if (result.meta_description) dispatch({ type: 'SET_AI_META_DESCRIPTION', value: result.meta_description });
      }
    } catch {
  ...
  }, [aiGenerate, defaultLocaleCode, t]);

  const handleRegenerateOutline = useCallback(async () => {
    dispatch({ type: 'SET_AI_ERROR', value: null });
    try {
      const result = await aiGenerate('draft_outline', stateRef.current.aiIdea, defaultLocaleCode);
      dispatch({
        type: 'OUTLINE_GENERATED',
        title: result.title || '',
        subtitle: result.subtitle || '',
  ...
  const handleInsertOutlineItem = useCallback((afterIndex: number) => {
    const newItem = { id: outlineIdCounter.current++, value: '' };
    const updated = [...stateRef.current.aiOutline];
    updated.splice(afterIndex + 1, 0, newItem);
    dispatch({ type: 'SET_AI_OUTLINE', value: updated });
  }, []);
  ...
  return (
    <FormDialog
      ...
      onClose={onClose}
      icon="article"
      title={t('blogs.wizard.title')}
      maxWidth={state.method === 'template' || state.method === 'ai' || (state.activeStep === 0 && aiAvailable) ? 'md' : 'sm'}
      data-testid="create-blog-wizard"
      actions={
    ...
    >
        <Stepper activeStep={state.activeStep} sx={{ mb: 1 }}>
        ...
        {state.activeStep === 1 && state.method === 'scratch' && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField autoFocus label={t('blogs.wizard.fields.title')} fullWidth required {...register('title')} error={!!errors.title} helperText={errors.title?.message} data-testid="create-blog-wizard.input.title" />
            <TextField label={t('blogs.wizard.fields.slug')} fullWidth required {...register('slug', { onChange: () => { slugManuallyEdited.current = true; } })} error={!!errors.slug} helperText={errors.slug?.message || t('blogs.wizard.fields.slugHint')} data-testid="create-blog-wizard.input.slug" />
        ...
        {state.activeStep === 1 && state.method === 'ai' && (
          <BlogWizardAiStep
            ...
            onInsertOutlineItem={handleInsertOutlineItem}
