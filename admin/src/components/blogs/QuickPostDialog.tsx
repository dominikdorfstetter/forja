...
export default function QuickPostDialog({ open, onClose }: QuickPostDialogProps) {
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      const result = await aiGenerate('draft_outline', state.idea, defaultLocaleCode);
      dispatch({
  ...
  const handleRegenerate = useCallback(async (field: RegenerateField) => {
    ...
    try {
      if (field === 'all') {
        ...
        const result = await aiGenerate('excerpt', s.body, defaultLocaleCode);
        if (result.excerpt) dispatch({ type: 'SET_EXCERPT', value: result.excerpt });
      } else if (field === 'seo') {
        const result = await aiGenerate('seo', s.body, defaultLocaleCode);
        if (result.meta_title) dispatch({ type: 'SET_META_TITLE', value: result.meta_title });
        if (result.meta_description) dispatch({ type: 'SET_META_DESCRIPTION', value: result.meta_description });
      }
    } catch {
  ...
  const handleRegenerateOutline = useCallback(async () => {
    dispatch({ type: 'SET_AI_ERROR', value: null });
    try {
      const result = await aiGenerate('draft_outline', stateRef.current.idea, defaultLocaleCode);
      dispatch({
        type: 'OUTLINE_GENERATED',
        title: result.title || '',
        subtitle: result.subtitle || '',
  ...
  const handleInsertOutlineItem = useCallback((afterIndex: number) => {
    ...
    const updated = [...stateRef.current.outline];
    updated.splice(afterIndex + 1, 0, newItem);
    dispatch({ type: 'SET_OUTLINE', value: updated });
  }, []);
  ...
  return (
    <FormDialog
      ...
      onClose={handleClose}
      icon="edit_note"
      title={t('quickPost.title', 'Quick post')}
      maxWidth="md"
    ...
    >
        <Stack spacing={2}>
          ...
          {state.mode === 'ai' && state.aiStep === 'idea' && (
            <>
              <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{t('quickPost.ai.ideaTitle')}</Typography>
              {defaultLocaleName && (
                <Alert severity="info" icon={false} data-testid="ai-language-notice">
                  {t('quickPost.ai.languageNotice', { language: defaultLocaleName })}
                </Alert>
              )}
                ...
                <Stack spacing={1}>
                  {state.outline.map((item, index) => (
                    <Stack key={item.id} direction="row" spacing={1} sx={{ alignItems: "center" }}>
                      ...
                      <Tooltip title={t('quickPost.ai.insertPoint', 'Insert point below')} arrow>
                        <IconButton size="small" onClick={() => handleInsertOutlineItem(index)} disabled={isGenerating} aria-label={t('quickPost.ai.insertPoint', 'Insert point below')} data-testid={`outline-insert-${index}`}>
                  ...
                  <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
                    ...
                    <Button size="small" startIcon={isGenerating ? <CircularProgress size={16} /> : <RefreshIcon />} onClick={handleRegenerateOutline} disabled={isGenerating} data-testid="regenerate-outline-btn">
                      {t('quickPost.ai.regenerateOutlineBtn', 'Regenerate Outline')}
                    </Button>
                  </Stack>
                </Stack>
              </Box>
