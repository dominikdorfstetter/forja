...
export default function SectionPreview({ section, localizations, selectedLocaleId }: SectionPreviewProps) {
  ...
  if (section.settings) {
    ...
    if (typeof s.style === 'string') settingsChips.push(s.style);
    if (typeof s.layout === 'string') settingsChips.push(s.layout);
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Chip label={t(`sectionEditor.typeNames.${section.section_type}`)} size="small" sx={{ bgcolor: borderColor, color: 'white' }} />
