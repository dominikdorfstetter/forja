...
beforeEach(async () => {
  ...
  vi.mocked(getSectionLocalizations).mockResolvedValue(mockLocalizations);
  vi.mocked(upsertSectionLocalization).mockResolvedValue({
    id: 'loc-new',
    page_section_id: 'section-1',
    locale_id: 'locale-en',
  });
  vi.mocked(updatePageSection).mockResolvedValue(undefined as never);
...
describe('SectionEditorDialog', () => {
  it('renders a single Save button (no Save All)', async () => {
    ...
    // Exactly one submit button — the primary Save in the footer.
    // Located via testid so we don't accidentally match the Material
    // Symbols "save" ligature that renders inside the button's text content.
    expect(screen.getByTestId('section-editor.btn.submit')).toBeInTheDocument();
  ...
  it('Save button saves all dirty locales and section config', async () => {
    ...
    // Click Save (explicit save — autosave has been removed)
    await user.click(screen.getByTestId('section-editor.btn.submit'));

    // Should save both locales
    await waitFor(() => {
      expect(upsertSectionLocalization).toHaveBeenCalledTimes(2);
    });

    // Should also save section config
    expect(updatePageSection).toHaveBeenCalledTimes(1);
  });
