...
interface UseListPageStateOptions {
  ...
  /** Initial chip filter id. Consumers use this with the list-page kit v2 chip bar. */
  initialChipFilter?: string;
}
...
export function useListPageState<T>(options?: UseListPageStateOptions) {
  ...
  const setChipFilter = useCallback((next: string) => {
    setChipFilterState(next);
    setPage(1);
  }, []);
