...
import { getSocialLinks } from '@/services/social';
...
describe('SocialLinksPage', () => {
  it('shows loading state initially', () => {
    vi.mocked(getSocialLinks).mockReturnValue(new Promise(() => {}));
    renderWithProviders(<SocialLinksPage />);
    expect(screen.getByRole('progressbar')).toBeInTheDocument();
  });

  it('renders social links table after data loads', async () => {
    vi.mocked(getSocialLinks).mockResolvedValue([mockLink, mockLink2]);
    renderWithProviders(<SocialLinksPage />);
    await waitFor(() => {
  ...
  it('shows empty state when no links', async () => {
    vi.mocked(getSocialLinks).mockResolvedValue([]);
    renderWithProviders(<SocialLinksPage />);
    await waitFor(() => {
  ...
  it('shows error alert when API fails', async () => {
    vi.mocked(getSocialLinks).mockRejectedValue(new Error('Network error'));
    renderWithProviders(<SocialLinksPage />);
    await waitFor(() => {
