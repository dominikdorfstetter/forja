...
import { getRedirects } from '@/services/redirects';
...
describe('RedirectsPage', () => {
  it('shows loading state initially', () => {
    vi.mocked(getRedirects).mockReturnValue(new Promise(() => {}));
    renderWithProviders(<RedirectsPage />);
    expect(screen.getByRole('progressbar')).toBeInTheDocument();
  });

  it('renders redirect table after data loads', async () => {
    vi.mocked(getRedirects).mockResolvedValue(mockPaginated);
    renderWithProviders(<RedirectsPage />);
    await waitFor(() => {
  ...
  it('shows empty state when no redirects', async () => {
    vi.mocked(getRedirects).mockResolvedValue(emptyPaginated);
    renderWithProviders(<RedirectsPage />);
    await waitFor(() => {
