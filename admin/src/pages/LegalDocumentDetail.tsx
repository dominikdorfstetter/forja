...
import { createLegalGroup, createLegalItem, deleteLegalGroup, deleteLegalItem, getLegalDocuments, getLegalGroups, getLegalItems, updateLegalDocument, updateLegalGroup, updateLegalItem } from '@/services/legal';
...
function GroupFormDialog({ open, group, onSubmit, onClose, loading }: GroupFormDialogProps) {
  ...
  return (
    <FormDialog
      ...
      loading={loading}
    >
      <TextField label={t('legalDetail.dialog.cookieName')} fullWidth size="small" {...register('cookie_name', { required: t('legalDetail.dialog.cookieNameRequired') })} error={!!errors.cookie_name} helperText={errors.cookie_name?.message} />
      <TextField label={t('legalDetail.dialog.displayOrder')} type="number" fullWidth size="small" {...register('display_order', { valueAsNumber: true })} />
...
export default function LegalDocumentDetailPage() {
  ...
  useEffect(() => {
    ...
    async function findDocument() {
      for (const site of sites!) {
        try {
          const result = await getLegalDocuments(site.id);
  ...
  const updateDocMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (sites) {
        for (const site of sites) {
          getLegalDocuments(site.id).then((result) => {
            const found = result.data.find((d) => d.id === id);
  ...
  return (
    <Box data-testid="legal-detail.page">
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Typography variant="body1" sx={{ fontFamily: "monospace" }}>{document?.cookie_name || '...'}</Typography>
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        {canWrite && <M3Button variant="outlined" size="sm" icon="add" onClick={() => dialogDispatch({ type: 'OPEN_GROUP_FORM' })}>{t('legalDetail.groups.add')}</M3Button>}
      </Box>
          ...
          <Accordion key={group.id} defaultExpanded>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%', mr: 2 }}>
                <Typography variant="subtitle1" sx={{ flexGrow: 1, fontFamily: "monospace" }}>{group.cookie_name}</Typography>
      ...
      <LegalDocumentFormDialog
        ...
        siteId=""
        document={document ? { id: id!, cookie_name: document.cookie_name, document_type: document.document_type, status: 'Draft', version: 1, created_at: '', updated_at: '' } : null}
