...
import { reorderMenuItems, reorderNavigationItems } from '@/services/navigation';
...
export function useNavigationDragDrop({
  ...
  const reorderMutation = useMutation({
    mutationFn: (reorderItems: ReorderTreeItem[]) => {
      if (selectedMenu) {
        return reorderMenuItems(selectedMenu.id, reorderItems);
      }
      return reorderNavigationItems(selectedSiteId, reorderItems.map(i => ({ id: i.id, display_order: i.display_order })));
    },
    onError: (error) => {
      showError(error);
