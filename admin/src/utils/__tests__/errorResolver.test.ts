...
describe('resolveError', () => {
  ...
  describe('ProblemDetails with entity_type (Phase 3 wire-shape)', () => {
    it('tracer bullet: ENTITY_NOT_FOUND + entity_type=blog resolves like the old BLOG_NOT_FOUND', () => {
      const error = {
        type: 'about:blank',
        title: 'Not Found',
        status: 404,
      ...
      expect(result.title).toBe('Resource not found');
      expect(result.detail).toBe('Blog not found');
      expect(result.action).toContain('deleted or moved');
      expect(result.code).toBe('ENTITY_NOT_FOUND');
    });

    it('ENTITY_SLUG_TAKEN + entity_type=blog resolves to the BLOG_SLUG_TAKEN message', () => {
      const error = {
        type: 'about:blank',
        title: 'Conflict',
        status: 409,
    ...
    it('ENTITY_SLUG_TAKEN + entity_type=page resolves to the PAGE_SLUG_TAKEN message', () => {
      const error = {
        type: 'about:blank',
        title: 'Conflict',
        status: 409,
      ...
      expect(result.title).toBe('Page slug already in use');
      expect(result.action).toBe('Choose a different slug.');
    });

    it('ENTITY_SLUG_TAKEN + entity_type=project falls back to the slug-taken pattern', () => {
      const error = {
        type: 'about:blank',
        title: 'Conflict',
        status: 409,
      ...
      expect(result.title).toBe('Slug already in use');
      expect(result.action).toBe('Choose a different slug.');
    });

    it('ENTITY_NOT_FOUND with no entity_type still resolves via the not-found pattern', () => {
      const error = {
        type: 'about:blank',
        title: 'Not Found',
        status: 404,
      ...
      expect(result.title).toBe('Resource not found');
      expect(result.detail).toBe('Resource missing');
    });

    it('falls back to ProblemDetails detail when neither code nor entity_type maps anywhere', () => {
      const error = {
        type: 'about:blank',
        title: 'Server Error',
        status: 500,
        ...
        detail: 'Server says something went sideways',
      };

      const result = resolveError(error);

      expect(result.title).toBe('Server Error');
      expect(result.detail).toBe('Server says something went sideways');
    });

    it('legacy BLOG_SLUG_TAKEN code still resolves correctly (backwards compatibility)', () => {
      const error = {
        type: 'about:blank',
        title: 'Conflict',
        status: 409,
