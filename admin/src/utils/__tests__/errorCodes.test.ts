...
describe('resolveErrorCode', () => {
  ...
  describe('entity_type composite lookup (Phase 3)', () => {
    it('resolves ENTITY_SLUG_TAKEN + entity_type=blog to the BLOG_SLUG_TAKEN message', () => {
      const result = resolveErrorCode('ENTITY_SLUG_TAKEN', 'blog');
      expect(result).not.toBeNull();
      expect(result!.message).toBe('Blog slug already in use');
      expect(result!.action).toBe('Choose a different slug.');
    });

    it('resolves ENTITY_SLUG_TAKEN + entity_type=page to the PAGE_SLUG_TAKEN message', () => {
      const result = resolveErrorCode('ENTITY_SLUG_TAKEN', 'page');
      expect(result).not.toBeNull();
      expect(result!.message).toBe('Page slug already in use');
    });

    it('resolves ENTITY_NOT_FOUND with any entity_type via the not-found pattern', () => {
      const result = resolveErrorCode('ENTITY_NOT_FOUND', 'cv_entry');
      expect(result).not.toBeNull();
      expect(result!.message).toContain('not found');
    });

    it('resolves ENTITY_NOT_FOUND with no entity_type via the not-found pattern', () => {
      const result = resolveErrorCode('ENTITY_NOT_FOUND');
      expect(result).not.toBeNull();
      expect(result!.message).toContain('not found');
    });

    it('falls through to slug-taken pattern when entity_type has no specific override', () => {
      const result = resolveErrorCode('ENTITY_SLUG_TAKEN', 'project');
      expect(result).not.toBeNull();
      expect(result!.message).toBe('Slug already in use');
    });
  });
  ...
  describe('unknown codes', () => {
    ...
    it('returns null for unknown code with unknown entity_type', () => {
      const result = resolveErrorCode('TOTALLY_UNKNOWN_CODE', 'mystery_entity');
      expect(result).toBeNull();
    });
  });
});
