...
The default storage provider writes files to a directory on the local filesystem. `tower_http::services::ServeDir` is nested into the Axum router to serve these files over HTTP.
...
3. The router mounts `ServeDir` at the configured base URL (default: `/uploads`) via `Router::nest_service`.
4. Public URLs are of the form `/uploads/<path>`.

S3-backed storage is exposed via the `/files/{*path}` proxy handler instead, so admin and template clients see a uniform same-origin URL whether the bytes live on disk or in a bucket.
