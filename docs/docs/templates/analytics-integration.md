...
The `@forjacms/analytics` library is a lightweight, privacy-first tracking script that records pageviews from your frontend site. It uses `navigator.sendBeacon()` for reliable delivery and never sets cookies or stores personal data.
...
```typescript
import { init } from '@forjacms/analytics';
...
```typescript
import { trackPageview } from '@forjacms/analytics';
