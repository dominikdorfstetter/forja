...
Forja uses a module system to control which features are available for each site. Modules let you enable only what you need -- a personal blog can skip legal documents and portfolio entries, while a full portfolio site can enable everything.
...
| **Portfolio** | Portfolio entries (work experience, education, skills) | Disabled |
...
| **Forms** | Structured data collection with custom fields, server-side validation, GDPR self-service, and retention policies. See [Forms](./content/forms). | Disabled |
...
| Change module settings | Admin, Owner |
