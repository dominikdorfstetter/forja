...
:::note
Bulk delete is a **soft delete** -- selected items move to [Trash](./trash) and can be restored for 30 days before automatic purge. Still, double-check the selection count in the confirmation dialog before proceeding, since published items go offline immediately.
:::
