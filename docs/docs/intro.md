...
Forja is built with a **Rust backend** (Axum 0.8 on `axum-server` with rustls), a **React admin dashboard** (Vite + MUI), and pluggable **frontend templates** (Astro, Next.js, etc.). It is designed for developers who want a fast, type-safe CMS they can self-host, extend, and integrate into their own stack.
...
| **Content types** | Blog posts, static pages, portfolio (CV entries, skills, projects), legal pages, documents |
...
| **RBAC** | Six roles (Owner > Admin > Editor > Author > Reviewer > Viewer) with optional editorial workflow |
...
| **Site modules** | Enable only the features you need per site (blog, pages, portfolio, legal, documents, AI) |
...
- **Backend** -- Rust with Axum 0.8, SQLx, PostgreSQL. Serves the JSON API at `/api/v1` and Swagger UI at `/api-docs` (split into a public consumer doc and an admin doc).
- **Admin** -- React SPA built with Vite, Material UI, React Query, react-hook-form, and zod. Served at `/dashboard` in production, `localhost:3000` in development.
...
- **[Admin Guide](./admin-guide/onboarding)** -- learn how to manage content through the dashboard.
