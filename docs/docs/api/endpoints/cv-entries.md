...
| GET | `/cv/{id}` | Read | Get CV entry by ID (lightweight) |
| GET | `/cv/{id}/detail` | Read | Get CV entry detail — identical shape to `/cv/{id}`; exists for route uniformity ([ADR 0003](https://github.com/dominikdorfstetter/forja/blob/main/docs/adr/0003-uniform-content-route-convention.md)) |
