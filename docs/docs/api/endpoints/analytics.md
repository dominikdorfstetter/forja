...
:::info
This endpoint is designed to be called from the `@forjacms/analytics` tracking library on your public site. It uses a Read-level API key so the same key that fetches content can also record pageviews.
:::
