...
- Some list endpoints accept an optional `locale_id` query parameter (UUID) that filters results to items with content in the specified locale. When this filter is applied, the pagination metadata (`total`, `total_pages`) reflects only the filtered set, not the full collection. See [Blogs](./endpoints/blogs) for an example.
