...
Forja supports native TLS termination via `axum-server` + `rustls` (`axum_server::bind_rustls`). When `TLS_CERT_PATH` and `TLS_KEY_PATH` are both set, the backend binds HTTPS directly without a reverse proxy.
