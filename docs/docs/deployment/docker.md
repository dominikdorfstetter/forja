...
The Dockerfile uses six stages to keep the final image small and speed up iterative builds:
...
| **runtime** | `debian:trixie-slim` | Minimal runtime with only `ca-certificates`, `libssl3t64`, and `libpq5` (Debian 13; `libssl3` was renamed `libssl3t64` in the 64-bit `time_t` transition) |

The cargo-chef pattern cuts full rebuild times from ~60 minutes to ~5 minutes for source-only edits by separating dependency compilation into a cached layer.
...
```json
{
  ...
  "services": [
    ...
    { "name": "redis", "status": "up", "latency_ms": 0 }
  ]
}
```
