...
The `closed` default closes a silent bypass: it prevents a Redis outage from letting unlimited traffic through for the duration of the outage.
:::
...
| `CLERK_EXPECTED_ISSUER` | -- | **Yes** (production with Clerk) | Expected `iss` claim on incoming Clerk JWTs, e.g. `https://clerk.your-app.com`. The boot guard requires this to be set explicitly in production — it does **not** auto-derive from `CLERK_PUBLISHABLE_KEY` / `CLERK_FAPI_DOMAIN`, so set it explicitly. The value is `https://<your-clerk-fapi-domain>` — the same host that appears in `CLERK_JWKS_URL`. |
